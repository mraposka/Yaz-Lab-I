...
# Get all csv files on dir
current_directory = os.getcwd()  # Geçerli çalışma dizini
csv_files = [f for f in os.listdir(current_directory) if f.endswith('.csv')]

# Informer Model (simplified version) https://github.com/zhouhaoyi/Informer2020/blob/main/models/model.py
class InformerModel(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.fc(lstm_out[:, -1, :])  # Take the last time step (idk how i found)
        return output


# Graph and prediction
def informerPrediction(time_data, label='Unknown'):
    ...
    if len(X) == 0:
        print(f"Insufficient data for {label} to create sequences") # safety first
        return
